...
let basketSearch = document.querySelector('.hidden');
document.querySelector('.cartIcon')
    .addEventListener('click', () =>
    {basketSearch.classList.remove("hidden")});

document.querySelector('body')
    .addEventListener('mouseout', () =>
    {basketSearch.classList.add("hidden")});
  ...
  const productRow = `
    <div class="basketRow" data-id="${productId}">
      ...
      <div>
        ...
        >${(basket[productId].price * basket[productId]
      .count).toFixed(2)}</span>
      </div>
    </div>
    `;
